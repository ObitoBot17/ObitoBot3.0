	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				success: '🤖PRONTO✔️',
				error: {
					stick: 'Falha, ocorreu um erro ao converter a imagem para o adesivo',
					Iv: 'LINK INVÁLIDO❌'
				},
				only: {
			...
			switch(command) {
				...
				case 'listadmins':
					...
					break
				case 'toimg':
					if (!isQuotedSticker) return reply('鉂� reply stickernya um 鉂�')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('鉂� Gagal, pada saat mengkonversi sticker ke gambar 鉂�')
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Sukses mengaktifkan mode simi di group ini 鉁旓笍')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Sukes menonaktifkan mode simi di group ini 鉁旓笍')
					} else {
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Sukses mengaktifkan fitur welcome di group ini 鉁旓笍')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Sukses menonaktifkan fitur welcome di group ini 鉁旓笍')
					} else {
						reply('1 untuk mengaktifkan, 0 untuk menonaktifkan')
					}
				case 'clone':
